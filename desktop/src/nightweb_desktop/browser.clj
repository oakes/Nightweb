...
(defn create-tab
  ...
  (let [new-tab (Tab. "Browser")
    ...
    (jfx/defhandler :onAction back-btn
                    (when (> (.getCurrentIndex history) 0)
                      (.go history -1)))
    (jfx/defhandler :onAction for-btn
                    (when (< (+ (.getCurrentIndex history) 1)
                             (.size (.getEntries history)))
                      (.go history 1)))
        ...
        (changed [ov oldState newState]
          (if (= newState javafx.concurrent.Worker$State/RUNNING)
            ...
            (do
              ...
              (->> (truncate-title (get-browser-title web-engine) 15)
                   (.setText new-tab)))))))
